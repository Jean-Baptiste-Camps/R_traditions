...
From secondary controlled data:
...
romans = read.csv("data/Pierandrea_table_p53.csv")
romans = romans[, "Nmss", drop = FALSE]
romans = cbind(romans, "romances")
colnames(romans) = c("Nwits", "Type")
epics = RepTradData
epics = cbind(epics, "epics")
colnames(epics) = c("Nwits", "Type")
data = rbind(epics, romans)
ggplot(data, aes(Nwits, fill = Type)) + 
        geom_bar(stat = "count", position = "dodge") + scale_fill_manual(values=cbPalette) # + scale_x_log10() 
